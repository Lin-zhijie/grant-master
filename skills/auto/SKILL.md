...
auto 在调用 subagent 时，**只允许使用 Grant-Master 定义的 worker agent 类型**。不同宿主的 agent 名称不同，但都必须映射到本仓库的 worker contract：

| 宿主 | Searcher | Digester | Writer |
|------|----------|----------|--------|
| Claude Code plugin | `grant-master:grant-searcher` | `grant-master:grant-digester` | `grant-master:grant-writer` |
| Codex multi-agent | `grant_searcher` | `grant_digester` | `grant_writer` |

Codex 运行前必须能通过 Codex agent 注册找到上述三个 role。Grant-Master 同时提供两层配置：

1. 仓库内 `.codex/config.toml`：作为 reference configuration，便于审计和测试；
2. 用户级 `~/.codex/config.toml`：由 `scripts/codex/register-agents.sh` 写入，保证用户在任意申请书项目目录运行时也能找到 worker。

三个 role 必须分别指向：

```text
.codex/agents/grant-searcher.toml  → agents/searcher.md
.codex/agents/grant-digester.toml  → agents/digester.md
.codex/agents/grant-writer.toml    → agents/writer.md
```

可用以下命令快速注册/修复 Codex agent：

```bash
bash scripts/codex/register-agents.sh
```

可用以下命令快速检查仓库内 agent 定义：
...
各 coordinator skill（03/04/08）在 dispatch 时必须使用对应宿主的 worker agent 类型**字符串字面量**，不允许动态推断或回退到通用型。

Codex 下，在第一次 dispatch worker 前必须执行一次注册自检：

1. 定位本 plugin 根目录（即包含 `scripts/codex/register-agents.sh` 的目录；从当前 `SKILL.md` 向上两级通常就是 plugin 根目录）；
2. 运行 `bash <plugin_root>/scripts/codex/register-agents.sh`；
3. 若脚本成功，继续使用 `grant_searcher` / `grant_digester` / `grant_writer`；
4. 若脚本不存在、执行失败，或执行后仍无法确认 worker 可用，必须生成 blocked 结果，并提示用户在插件根目录手动运行 `bash scripts/codex/register-agents.sh` 后重启 Codex 桌面端或开启新会话。
