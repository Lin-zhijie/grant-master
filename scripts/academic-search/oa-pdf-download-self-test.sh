...
# ── YAML input test ──────────────────────────────────────────────
INPUT_YAML="${TMP_DIR}/papers.yaml"
MANIFEST_YAML="${TMP_DIR}/manifest_yaml.json"
cat > "${INPUT_YAML}" <<YAML
- title: "Open Paper"
  authors: ["A. Researcher"]
  year: 2026
...
YAML_MANIFEST_ONLY="$(node "${ROOT_DIR}/scripts/oa-pdf-download.mjs" --input "${INPUT_YAML}" --manifest "${MANIFEST_YAML}")"
assert_contains "${YAML_MANIFEST_ONLY}" '"eligible":2' "YAML input: manifest eligible count"
assert_contains "${YAML_MANIFEST_ONLY}" '"skipped":1' "YAML input: manifest skipped count"
[ -s "${MANIFEST_YAML}" ] || fail "YAML input: manifest file not created"
